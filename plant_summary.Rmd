...
Herbaria are valuable resources for documenting biology in an era of human impact on ecosystems [@heberling2019].
Although herbarium use began with research subjects such as taxonomy and local history, emergent fields of study such as bioinformatics and global change biology can use herbarium resources to extend studies across time and space. Today, 390 million specimens are contained within more than 3000 herbaria worldwide [@heberling2019].

This project aimed to augment the student collection at the Frostburg State University herbarium by collecting high-quality examples of local biodiversity.

# Methods

## Collections
I collected specimens from public, non-protected areas and from private lands where permission was obtained from the landowner. For each specimen, I identified the species and recorded locality, habitat, coordinates, and three or more associated species. I pressed and mounted vouchers following University of Florida guidelines [@flas].

Using the data associated with each collection, I created a spreadsheet of plant specimens. Digitizing this information allowed me to use code to generate herbarium labels, map specimen locations, and automate certain summary text. All coding was conducted in program R version `r glue("{version$major}.{version$minor}")` [@rcoreteam2023].
Code associated with this project is available in a GitHub repository at https://github.com/mlmassa/PlantTax.
...
This `r str_to_sentence(plants$common_name[x])` was collected `r format(plants$date[x], "%B %e")` from `r plants$site[x]` in `r plants$county[x]` County, `r plants$state[x]` (`r round(plants$lat[x], 5)`, `r round(plants$long[x], 5)`).
...
**Range and habitat.**
This species is `r if_else(plants$native[x] == "Y", "native", "not native")` to Maryland.
...
Fertile blades have contrasted distal pinnae, upon which are confluent sori covering the abaxial surface which are covered by entire, deciduous peltate indusia.
The spores are small and uniform in size [@murrell2010].
...
**Economic and cultural significance.**
This species is also a popular ornamental garden planting and indoor plant [@murrell2010].
...
**Range and habitat.**
This species is `r if_else(plants$native[x] == "Y", "native", "not native")` to Maryland.
It ranges throughout eastern North America and can be found in shaded coniferous and mixed hardwood forests [@fna], mainly on forest soils rather than rock outcrops [@fov].
...
Each blade has 10-40 pairs of alternately-arranged pinnae [@hilty2002].
Sporangia are linear and elongate.
This specimen possesses sori, some with indusia which are translucent and whitish [@fov].
Spores are wind-dispersed in summer or early fall [@hilty2002].
...
Animal associations are mostly unknown, although aphids and woodland jumping mice are known to feed on the juices and fronds respectively [@hilty2002].

Reticulate evolution, the process by which new species arise through merging of lineages, occurs in *Asplenium* ferns [@werth1985].
Hybridization between species in this genus is a widespread phenomenon, particularly in Appalachia. *A. platyneuron* is known to create fertile hybrids with *A. montanum* (creating Bradley's spleenwort, *A. bradleyi*), *A. rhizophyllum* (creating Tutwiler's spleenwort *A. tutwilerae*), and with their hybrid lobed spleenwort *A. pinnatifidum* (creating Kentucky spleenwort *A. x kentuckiense*) [@knouse]. *A. tutwilerae* can then backcross with *A. platyneuron*.
Those hybrids that are fertile are allotetraploids, having experienced a chromosome doubling event, possibly multiple times. The backcross hybrids are triploid and sterile.
...
The leaves are glossy and evergreen, lanceolate or elliptic, broadest near the middle, and rounded at the base, which distinguishes this species from the larger, ovate-leaved *V. major* [@fov]. The flowers are blue to blue-violet, 5-parted, and perfect [@murrell2010].

**Range and habitat.**
It is locally frequent in disturbed forests and clearings in New England, the Appalachian mountains, the Midwest, the Southeast, and parts of the Pacific Northwest.

**Ecology.**
A non-native species, *V. minor* is assigned a low invasiveness rank, but under the right conditions it can cover large areas and threaten native forest ground flora. Its ability to rapidly vegetatively propogate from fragments and yard waste is key to its invasive potential. Although in its native range periwinkle seeds are usually dispersed by ants, it typically does not spread by seed in its introduced range [@stone2009].
...
**Ecology.**
Although it can self-pollinate, it is primarily insect-pollinated by bees, hoverflies, flies, beetles, and ants [@innes2011].
In its native range, 21% of seeds were lost to predation by animals.

**Economic and cultural significance.**
This species is used as an expectorant and cough remedy. The genus name comes from the Latin word for cough, tussis.
...
**Range and habitat.**
This species is `r if_else(plants$native[x] == "Y", "native", "not native")` to Maryland.
*Carex pensylvanica* can be found in varied habitats, from dry to mesic forests, woodlands, barrens, balds, and clearings. It is abundant in rocky or shale habitats and is not limited by soil chemistry [@fov]. It prefers sandy or loose loam with organic matter [@hilty2002].

**Ecology.**
A variety of insects feed on this species, including leafhoppers, aphids, grasshoppers, and grass-miner moths [@hilty2002]. The mature seeds are commonly consumed by voles and birds including sparrows, Wild Turkey, and immature Ruffed Grouse. 

As habitat, it creates cover and nesting material for migratory waterfowl as well as prairie chickens [@cope1992]. Its dominance in certain ecosystems can help keep succession at bay. In burned ecosystems, it can exploit openings after low-intensity fire to propogate clonally.

**Economic and cultural significance.**
This species can form turf as it extends its rhizomes to form new clumps, making it a good ground cover for native plant gardens and yards [@hilty2002].
...
**Morphological description.**
*Equisetum arvense* is a widespread horsetail. The vegetative stems are greed, branched in whorls with a hollow center, and grow from 2-60 (sometimes to 100) cm [@fna]. The stem has squarish sheaths with dark teeth. The fertile stems are brown, unbranched, larger-sheathed, fleshy, and ephemeral. The strobilus consists of a terminal, ovoid structure with peltate sporangiophores.

**Range and habitat.**
This species is `r if_else(plants$native[x] == "Y", "native", "not native")` to Maryland.
The species ranges throughout North America from Alaska and Nunavut down to New Mexico, northern Texas, Tennessee, and North Carolina [@fna]. It can be found in a variety of habitats including roadsides, riverbanks, fields, marshes, pasture, and tundra.

**Ecology.**
*Equisetum* is likely the most ancient extant genus of vascular plants, being a dominant coal-producing flora during the Carboniferous [@murrell2010].
...
**Economic and cultural significance.**
This species is not economically important today, but the silicon dioxide crystals incorporated into this cells of plants in this genus give them the name "scouring-rush" due to their usefulness in cleaning cookware [@murrell2010]. Its weediness in poorly-drained pastures can make it an enemy of ranchers, because some members of this genus can be toxic to cattle.
...
Flowers are perfect, lemon-yellow, often with brown lines, with lateral 2 petals bearded [@fna].
There are three connate carpels and a superior, one-loculed ovary [@murrell2010].
The fruit is a glabrous, ovoid capsule containing bronze seeds.

**Range and habitat.** 
This species is `r if_else(plants$native[x] == "Y", "native", "not native")` to Maryland.
It is restricted to the eastern United States from New York to Alabama excluding Florida, growing in the mountains and Piedmont [@fna].
...
**Ecology.**
As with many members of *Viola*, the species possesses insect-pollinated chasmogamous flowers in spring as well as underdeveloped self-pollinated cleistogamous flowers produced in summer.
...
**Economic and cultural significance.**
While this species is not economically valuable, other members of *Viola* are popular ornamental plantings including pansies and sweet violets, also used in perfumes [@murrell2010].
...
**Morphological description.**
This lycopode exhibits tightly appressed leaves that are linear to narrowly lanceolate [@fna]. Successive branching leads to flat, fanlike branchlets. The strobili are on long stalks that are forked. The peduncles are dark green and glabrous with sparse, scale-like bracts [@hilty2002]. Each strobilus possesses appressed sporophylls, each 2mm in length, with a sporandium behind it. Spores are released in late summer or autumn. Notably, the horizontal stems lie aboveground in the leaf litter, not beneath the soil.
...
**Ecology.**
It is the most abundant *Diphasiastrum* species on the continent [@fna]. Even so, reproduction from spores is slow and may take 20 years [@hilty2002]. Dense clubmosses, including this species, are used as nesting substrate by Nashville Warbler.
...
It is common to rare depending on region, and can be found in rich cove forests, northern hardwoods, and well-drained floodplains, often in calcareous soils [@fov]. 

This species is ranked S3 in Maryland (rare or uncommon in the state, with 21-100 occurrences) and is tracked by the Maryland Natural Heritage Program as a species of conservation concern, but ranked G5 (demonstrably secure) globally. Given its status, I should not have collected it, but it was locally fairly common and the Flora of Virginia did not list it as a species of conservation concern.
...
Flowers are four-parted, small (6-21 mm), usually solitary, and can be terminal or axillary.
...
A variety of other pollinators visit the flowers, including carpenter bees, metallic Halictid bees, and butterflies, feeding on nectar and pollen [@hilty2002].
...
**Range and habitat.**
This species is `r if_else(plants$native[x] == "Y", "native", "not native")` to North Carolina, but not found in Maryland.
...
This species was in moist, rocky clay soils in a shaded understory (Figure \@ref(fig:digging)).
...
**Economic and cultural significance.**
This species has been used by the Catawba for stomach pain, heart trouble, and backaches, and by the Rappahanock for treatment of asthma and whooping cough [@fna].

```{r digging, fig.cap = "The author digging up *Hexastylis arifolia*. Photo by EG Eichenberger."}
knitr::include_graphics("data/processed/me_digging_hexastylis.jpg")
```
...
It produces 1-5 nodding, umbellate white to pinkish flowers that mature into capsules on erect reddish peduncles [@fov].
...
**Ecology.**
Insect-pollinated, primarily by bumblebees but also by honeybees, who feed on the nectar [@hilty2002]. Its other faunal associations are limited. It is not typically foraged by white-tailed deer, and is considered toxic to sheep.

After hemlock woolly adelgid infestation, *C. maculata* doubled in frequency in the understory of one forest in southern new England [@small2005]. This may be due to canopy thinning, as *C. maculata* prefers partial sun [@hilty2002]. Successful cultivation of this species relies on the presence of mycorrhizal fungi in the soil.
...
Its ethnobotanical uses include treatment of gallstones, kidney stones, and rheumatism by the Creek, whose word for the plant is "pipsisikweu" meaning "to break into small pieces" [@hiltonpond2001].
...
**Economic and cultural significance.**
Vineyards and homeowners alike grow muscadine grape for processing into wine, juice, and jellies. Many cultivars exist.
This species was the first native grape to be cultivated in North America [@andersen2020].
...
It is common component of dry to dry-mesic upland forests and woodlands in the eastern and central US. 
...
Black oak can suffer from oak wilt [@carey1992]. It is more susceptible than white oak, which is replacing it in certain areas. Spongy moth (*Lymantria dispar*) defoliation can also kill these trees. A combination of multiple factors including spongy moth, drought, and other boring insects can lead to stand mortality.

This species is moderately fire-resistant, with young trees resprouting after being top-killed and older trees surviving because of thick bark. Fire suppression has reduced the prevalence of black oak in oak-hickory forests.

Hybrids are known with a variety of other oaks, including scarlet oak, northern pin oak, pin oak, and willow oak [@carey1992].

**Economic and cultural significance.** 
Black oak is also used for timber and used in furniture, flooring, and railroad ties, though it usually called "red oak" [@carey1992]. Tannins in the bark can be extracted at commercial scale.
...
Stems are diffuse and spreading-ascending, with green, linear leaves growing 1-2.5 cm in length and 1-2.4 mm in width [@fov].
Seeds are arils.

**Range and habitat.** 
This species is `r if_else(plants$native[x] == "Y", "native", "not native")` to Maryland.
The species grows naturally on rocky areas such as cliffs, bluffs, boulder fields, forests, and seepage swamps, often in association with calcareous or mafic substrate [@fov].
It can be found throughout the northern Appalachians, Great Lakes region, and southeastern Canada.
It is an obligate climax species, absent in early and mid-seral communities [@sullivan1993].

**Ecology.**
Moose browse Canada yew heavily, and white-tailed deer will eat it in the winter [@sullivan1993]. Browsing can extirpate the species, as nearly occurred on Isly Royale after the arrival of moose.

Arils are consumed by Ruffed Grouse and frugivorous songbirds such as Cedar Waxwing and American Robin. The seeds are then dispersed after being eaten.

**Economic and cultural significance.**
Native Americans in Michigan and Quebec have used the foliage of Canada yew to brew a beverage [@sullivan1993].
